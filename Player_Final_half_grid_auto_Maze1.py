        ...
        try:
            ...
            if len(good_matches) > 0:
                match_img = cv2.drawMatches(
                    best_gray1, best_kp1, 
                    best_gray2, best_kp2, 
                    best_good_matches[:min(len(good_matches), 50)], 
                    None, 
        ...
        try:
            ...
            for idx in indices[0]:
                ...
                if match_result['match_ratio'] > best_match_ratio:
                    ...
                    best_good_matches = match_result['matches']
                    gray1 = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if len(img.shape) == 3 else img
                    gray2 = cv2.cvtColor(matching_image, cv2.COLOR_BGR2GRAY) if len(matching_image.shape) == 3 else matching_image
                    kp1, kp2 = self.sift.detectAndCompute(gray1, None)[0], self.sift.detectAndCompute(gray2, None)[0]
                    best_kp1, best_kp2 = kp1, kp2
                    best_gray1, best_gray2 = gray1, gray2
            ...
            if best_match_index is not None:
                ...
                # Display matches using cv2.drawMatches for the best match
                    # if len(best_good_matches) > 0:
                    #     match_img = cv2.drawMatches(
                    #         best_gray1, best_kp1,
                    #         best_gray2, best_kp2,
                    #         best_good_matches[:min(len(best_good_matches), 50)],
                    #         None,
                    #         flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS
                    #     )
                    #     cv2.imshow('Image Matches', match_img)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key==97:
                        ...
                        current_pixel = (int(self.loc_cod[str(self.curr)][0]), int(self.loc_cod[str(self.curr)][1]))
                        cv2.circle(maze_solver_obj.img, current_pixel, radius=5, color=(150,150,150), thickness=4)  # Blue dot
