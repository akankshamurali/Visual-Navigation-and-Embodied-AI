...
class maze_solver():
    ...
    def drawPath(self, path, thickness=1):
        ...
        cv2.circle(self.img, self.src, 2, 120, 8)
        cv2.circle(self.img, self.dst, 2, 120, 8)
        x0, y0 = path[0]
        for vertex in path[1:]:
            x1, y1 = vertex
            cv2.line(self.img, (x0, y0), (x1, y1), 120, thickness)
            x0, y0 = vertex
    ...
    def proc_img(self, img_path: str = None, image=None, show_img=False, save_img=False, dilatation_size=2):
        ...
        if save_img:
            plt.imsave(
                f'img/maze_proc_{strftime("%Y%m%d_%H%M%S")}.png', inv_cropped_img)
