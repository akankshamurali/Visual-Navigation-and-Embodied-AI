...
class maze_solver():
    def __init__(self, src, dst, img=None, img_path=None, dilatation_size=2):
        if img.all() != None:
            self.img = self.proc_img(
                image=img, dilatation_size=dilatation_size)
        else:
            self.img = self.proc_img(
                image_path=img_path, dilatation_size=dilatation_size)
        self.src = src
        self.dst = self.grid_to_pixel(dst[0],dst[1])
    ...
    def drawPath(self, path, thickness=1):
        ...
        cv2.circle(self.img, self.src, 2, (0, 255, 0), 8)
    ...
    def proc_img(self, img_path: str = None, image=None, show_img=False, save_img=False, dilatation_size=2):
        ...
        if save_img:
            plt.imsave(
                f'img/maze_proc_{strftime("%Y%m%d_%H%M%S")}.png', resized)

        return resized

    def gen_path(self, p):
        # Full grid size params
        # GRID_X_ZERO = -7
        # GRID_Y_ZERO = -14
        # GRID_SHAPE = 15
        ...
        GRID_SHAPE = 30
        p = p[::-1]
        p_grid_coordinates = np.hstack((((np.array(p)[:, 0] // (self.img.shape[1]/GRID_SHAPE)) + GRID_X_ZERO).reshape(-1, 1), np.abs(
            (np.array(p)[:, 1] // (self.img.shape[0]/GRID_SHAPE)) + GRID_Y_ZERO).reshape(-1, 1)))
        p_grid_coordinates_unique = p_grid_coordinates[np.concatenate(
            ([True], np.any(p_grid_coordinates[1:] != p_grid_coordinates[:-1], axis=1)))]
        p_grid_coordinates_unique = np.vstack([[0, 0], p_grid_coordinates_unique])
        return (p_grid_coordinates_unique)
    ...
    def grid_to_pixel(self, gx, gy):
        ...
        cell_width = self.img.shape[1] / GRID_SHAPE
        cell_height = self.img.shape[0] / GRID_SHAPE

        # Invert the x-axis computation
        # gx = floor(px/cell_width) + GRID_X_ZERO
        # => px center = ( (gx - GRID_X_ZERO) + 0.5 ) * cell_width
        px = ((gx) + 0.5) * cell_width

        # Invert the y-axis computation
        # gy = abs(floor(py/cell_height) + GRID_Y_ZERO) = 22 - (py//cell_height)
        # => (py//cell_height) = 22 - gy
        # py center = ( (22 - gy) + 0.5 ) * cell_height
        py = ((abs(GRID_Y_ZERO) - gy) + 0.5) * cell_height
...
if __name__ == '__main__':
    ...
    GRID_Y_ZERO = -29
    GRID_SHAPE = 30
    p_grid_coordinates = np.hstack((((np.array(p)[:, 0] // (maze_solver_obj.img.shape[1]/GRID_SHAPE)) + GRID_X_ZERO).reshape(-1, 1), np.abs(
        (np.array(p)[:, 1] // (maze_solver_obj.img.shape[0]/GRID_SHAPE)) + GRID_Y_ZERO).reshape(-1, 1)))
    p_grid_coordinates_unique = p_grid_coordinates[np.concatenate(
        ([True], np.any(p_grid_coordinates[1:] != p_grid_coordinates[:-1], axis=1)))]
    p_grid_coordinates_unique = np.vstack([[0, 0], p_grid_coordinates_unique])
    print(p_grid_coordinates_unique)
